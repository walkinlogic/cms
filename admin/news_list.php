<?php 
	...
	if(isset($_GET['args1'])){
		...
		$mysql->record_update("news_tbl",array('status' => $args2),"id=$args1");
		//print_r($check);
	}	
	if(isset($_GET['args3'])){
		$args3 =cleaninputfield($mysql,$_GET['args3']);
		$mysql->record_delete("news_tbl","id = $args3");
	}
	?>
		...
		<div class="container">
			...
			<table width="100%" border="0" cellspacing="0" cellpadding="0" class="tbl">
				  <tr class="headerbar">
					...
					<th width="22%" class="alignleft">Date</th>
				  ...
				  <?php 
					$i=1;
					$tbl_name="news_tbl";		//your table name
					...
					/* Get data. */
					$postrows = $mysql->list_table("$tbl_name",false, array ('range' => '*','limitOffset'=>$start,'rowCount'=>$limit,'sortColumn'=>"newsdate",'sortType'=>'DESC'));
					...
					<td class="alignleft"><?php echo $postrow['newsdate']; ?></td>
					<td align="left">
					<?php 
						$status=$postrow['status'];
						if($status == 1){ 
							echo "<a href='news_list.php?args1=$postrow[id]&args2=0'> &nbsp; &nbsp; <img src='images/active.png' border='0' alt='active' /> Active &nbsp; &nbsp;</a>";
						}else{ 
							echo "<a href='news_list.php?args1=$postrow[id]&args2=1'> &nbsp; &nbsp; <img src='images/hidden.png' border='0' alt='hidden' /> Inactive</a>";
						}	
						?>						</td>
					<td><a href="new_news.php?args1=<?php echo $postrow['id']; ?>"><img src="images/edituser.png" border="0" alt="edit user" title="Edit" /></a></td>
					<td><a href="news_list.php?args3=<?php echo $postrow['id']; ?>" onclick="return confirm('Do you want to delete this?');"><img src="images/delete.png" border="0" title="Delete" alt="delete user" /></a></td>
				  </tr>
				  <?php 
				  		$i++;
				  		} ?>
			...
			<div class="addbtn-right">
				<a href="new_news.php" title="Add News"><input name="add" id="add" border="0" type="submit" value="Add News" class="addpagebtn" /></a> 
			</div>
		</div>
