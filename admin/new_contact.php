	...
	<div class="wrap">
		...
		<?php 
			if(isset($_POST['add'])){
				...
				$queryinsert = $mysql->record_insert("contactdetails_tbl",array('ar_files' => $ar_pdffilename,'files' => $pdffilename,'ar_title' => $ar_title,'ar_description' => $ar_description,'description' => $description ,'title' => $title,'link' => $link ,'image' => $imagename,'ar_image' => $ar_imagename,'sortorder' => $sortorder),false);
				if($queryinsert){
					$msg= base64_encode('Custom Image Added Successfully!');
					echo "<script> window.location='contacts.php?msg=$msg';</script>";
					exit();
				}else{
			...
			if(isset($_POST['update'])){
				...
				if($_FILES['ar_pdffile']['tmp_name']!=''){
				 ...
				 $description =cleaninputfield($mysql,$_POST['description']);				
				 $ar_description =cleaninputfield($mysql,$_POST['ar_description']);
				$queryupdate = $mysql->record_update("contactdetails_tbl",array('ar_files' => $ar_pdffilename,'files' => $pdffilename,'ar_description' => $ar_description,'description' => $description ,'title' => $title,'ar_title' => $ar_title,'link' => $link ,'ar_image' => $ar_imagename,'image' => $imagename,'sortorder' => $sortorder),"id=$imageid");
				if($queryupdate){
			...
			if(isset($_GET['args1'])){
				...
				$postrow = $mysql->fetch_row("contactdetails_tbl","id=$args1");
			}
			?>
				   ...
				   <tr>
				     ...
				     <td><?php 
						if(isset($postrow)){
							...
							$max_value = $mysql->table_max_value("contactdetails_tbl","sortorder"); 
							$max_value += 1;
						}
						?>
