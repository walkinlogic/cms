	...
	<div class="wrap">
		...
		<?php 
			if(isset($_POST['addnews'])){
				...
				$queryinsert = $mysql->record_insert("news_tbl",array('pageid' => $link,'ar_image' => $ar_imagename,'image' => $imagename,'heading' => $heading,'ar_heading' => $ar_heading,'newsdate' => $newsdate,'summary' => $summary,'ar_summary' => $ar_summary,'description' => $description,'ar_description' => $ar_description,'status' => $status),false);
				if($queryinsert){
					$msg= base64_encode('News Added Successfully!');
					echo "<script> window.location='news_list.php?msg=$msg';</script>";
					exit();
				}else{
...
/* dirname(dirname(__FILE__)). */				
				...
				$queryupdate = $mysql->record_update("news_tbl",array('pageid' => $link,'ar_image' => $ar_imagename,'image' => $imagename,'heading' => $heading,'ar_heading' => $ar_heading,'newsdate' => $newsdate,'summary' => $summary,'ar_summary' => $ar_summary,'description' => $description,'ar_description' => $ar_description,'status' => $status),"id=$newsid");
				if($queryupdate){
			...
			if(isset($_GET['args1'])){
				...
				$postrow = $mysql->fetch_row("news_tbl","id=$args1");
			}
			?>
	...
	<?php 
	  ...
	  <?php }else{ ?>
				...
				<input name="add" id="add" border="0" type="submit" value="Add News" class="addpagebtn" />
		<?php } ?>
			</div>
		</div>
		</form>	
