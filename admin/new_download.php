	...
	<div class="wrap">
		...
		<?php 
			if(isset($_POST['add'])){
				...
				$queryinsert = $mysql->record_insert("downloads_tbl",array('download_art_value' => $download_art_value,'ar_files' => $ar_pdffilename,'files' => $pdffilename,'ar_title' => $ar_title,'ar_description' => $ar_description,'description' => $description ,'title' => $title,'link' => $link ,'image' => $imagename,'ar_image' => $ar_imagename,'sortorder' => $sortorder),false);
				if($queryinsert){
					$msg= base64_encode('Custom Image Added Successfully!');
					echo "<script> window.location='downloads.php?msg=$msg';</script>";
					exit();
				}else{
			...
			if(isset($_POST['update'])){
				...
				if($_FILES['ar_pdffile']['tmp_name']!=''){
				 ...
				 $description =cleaninputfield($mysql,$_POST['description']);				
				 $ar_description =cleaninputfield($mysql,$_POST['ar_description']);
				$queryupdate = $mysql->record_update("downloads_tbl",array('download_art_value' => $download_art_value,'ar_files' => $ar_pdffilename,'files' => $pdffilename,'ar_description' => $ar_description,'description' => $description ,'title' => $title,'ar_title' => $ar_title,'link' => $link ,'ar_image' => $ar_imagename,'image' => $imagename,'sortorder' => $sortorder),"id=$imageid");
				if($queryupdate){
			...
			if(isset($_GET['args1'])){
				...
				$postrow = $mysql->fetch_row("downloads_tbl","id=$args1");
			}
			?>
		...
		<div class="container">
			...
			<table width="100%" border="0" cellspacing="0" cellpadding="0" class="addpage">
				  ...
				  <tr>
					<td width="11%"><label for="ar_title">Choose Type</label></td>
					<td width="89%">
							...
							<?php 
								...
								if($mysql->affected_rows>0){
									foreach($rows as $value => $row){ 
										...
										$sel_1 = (isset($postrow) && $row['id'] == $postrow['link']) ? "selected='selected'" : "";
									?>
				   ...
				   <tr>
				     ...
				     <td><?php 
						if(isset($postrow)){
							...
							$max_value = $mysql->table_max_value("downloads_tbl","sortorder"); 
							$max_value += 1;
						}
						?>
				...
				<tr>
				  ...
				  <tr>
					...
					<td>&nbsp;<input type="file" name="pdffile" id="pdffile" /><?php  if(isset($postrow)){?> <input type="hidden" name="oldpdffile"  id="oldpdffile" value="<?php echo $postrow['files'] ?>" /> <?php echo "(   $postrow[files]   )"; } /// end of the file uploading updation ?>
					</td>
				  </tr>	
				  <tr>
					<td width="11%"><label for="image">Arabic Image </label></td>
