	...
	<div class="wrap">
		...
		<?php 
			if(isset($_POST['add'])){
				...
				$queryinsert = $mysql->record_insert("customimages_tbl",array('ar_title' => $ar_title,'ar_description' => $ar_description,'description' => $description ,'title' => $title,'link' => $link ,'image' => $imagename,'sortorder' => $sortorder),false);
				if($queryinsert){
					$msg= base64_encode('Custom Image Added Successfully!');
					echo "<script> window.location='custom_images.php?msg=$msg';</script>";
					exit();
				}else{
			...
			if(isset($_POST['update'])){
				...
				if($_FILES['image']['tmp_name']!=''){
				 ...
				 $description =cleaninputfield($mysql,$_POST['description']);				
				 $ar_description =cleaninputfield($mysql,$_POST['ar_description']);
				$queryupdate = $mysql->record_update("customimages_tbl",array('ar_description' => $ar_description,'description' => $description ,'title' => $title,'ar_title' => $ar_title,'link' => $link ,'image' => $imagename,'sortorder' => $sortorder),"id=$imageid");
				if($queryupdate){
		...
		<div class="container">
			...
			<table width="100%" border="0" cellspacing="0" cellpadding="0" class="addpage">
				   ...
				   <tr>
				     ...
				     <td><?php 
						if(isset($postrow)){
							...
							$max_value = $mysql->table_max_value("customimages_tbl","sortorder"); 
							$max_value += 1;
						}
						?>
			  ...
			  <tr>
				  ...
				  </tr>			<tr>			<td valign="top"><label for="description">Top Text</label></td>			
