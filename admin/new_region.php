	...
	<div class="wrap">
		...
		<?php 
			if(isset($_POST['add'])){
				...
				$queryinsert = $mysql->record_insert("customregion_tbl",array('ar_title' => $ar_title,'ar_description' => $ar_description,'description' => $description ,'title' => $title,'link' => $link ,'image' => $imagename,'sortorder' => $sortorder),false);
				if($queryinsert){
					$msg= base64_encode('Custom regions Added Successfully!');
					echo "<script> window.location='custom_regions.php?msg=$msg';</script>";
					exit();
				}else{
			...
			if(isset($_POST['update'])){
				...
				if($_FILES['image']['tmp_name']!=''){
				 ...
				 $description =cleaninputfield($mysql,$_POST['description']);				
				 $ar_description =cleaninputfield($mysql,$_POST['ar_description']);
				$queryupdate = $mysql->record_update("customregion_tbl",array('ar_description' => $ar_description,'description' => $description ,'title' => $title,'ar_title' => $ar_title,'link' => $link ,'image' => $imagename,'sortorder' => $sortorder),"id=$imageid");
				if($queryupdate){
					$msg= base64_encode('Custom regions Updated Successfully!');
					echo "<script> window.location='custom_regions.php?msg=$msg';</script>";
					exit();
				   ...
				   <tr>
				     ...
				     <td><?php 
						if(isset($postrow)){
							...
							$max_value = $mysql->table_max_value("customregion_tbl","sortorder"); 
							$max_value += 1;
						}
						?>
