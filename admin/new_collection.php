	...
	<div class="wrap">
		...
		<?php 
			if(isset($_POST['adds'])){
				...
				$queryinsert = $mysql->record_insert("collections_tbl",array('name' => $name,'ar_name' => $ar_name,'meta_title'=>$meta_title,'ar_meta_title'=>$ar_meta_title,'meta_keyword' => $meta_keyword,'ar_meta_keyword' => $ar_meta_keyword,'meta_desc' => $meta_desc,'ar_meta_desc' => $ar_meta_desc,'sort_order' => $sort_order,'image' => $imagename,'image2' => $imagename2,'status' => $status,'description' => $description,'ar_description' => $ar_description),false);
				if($queryinsert){
					$msg= base64_encode('Collection Added Successfully!');
					echo "<script> window.location='collections_list.php?msg=$msg';</script>";
					exit();
				}else{
					$msg= base64_encode(mysqli_error($mysql->connection));
					echo "<script> window.location='collections_list.php?msge=$msg';</script>";
					exit();
				}		
			}
			...
			if(isset($_POST['updates'])){
				...
				$queryupdate = $mysql->record_update("collections_tbl",array('name' => $name,'ar_name' => $ar_name,'meta_title'=>$meta_title,'ar_meta_title'=>$ar_meta_title,'meta_keyword' => $meta_keyword,'ar_meta_keyword' => $ar_meta_keyword,'meta_desc' => $meta_desc,'ar_meta_desc' => $ar_meta_desc,'sort_order' => $sort_order,'image' => $imagename,'image2' => $imagename2,'status' => $status,'description' => $description,'ar_description' => $ar_description),"id=$ids");
				if($queryupdate){
			...
			if(isset($_GET['args1'])){
				...
				$postrow = $mysql->fetch_row("collections_tbl","id=$args1");
			}
			?>
			...
			<table width="100%" border="0" cellspacing="0" cellpadding="0" class="addpage">
				...
				<tr>
				  ...
				  <tr>
					...
					<?php 
						if(isset($postrow)){
							...
							$max_value = $mysql->table_max_value("collections_tbl","sort_order"); 
							$max_value += 1;
						}
						?>
				  ...
				  <tr>
					...
					<td>&nbsp;<input type="file" name="image" id="image"  <?php  if(!isset($postrow)){?> class="required" <?php } ?> /><?php  if(isset($postrow)){?> <input type="hidden" name="oldimage"  id="oldimage" value="<?php if(isset($postrow)){echo $postrow['image'];} ?>" /> <?php echo "(   $postrow[image]   )"; } /// end of the file uploading updation ?>
					</td>
				  </tr>
				  	<tr>
				  ...
				  <tr>
					<td valign="top"><label for="description">Detail</label></td>
				  ...
				  <tr>
					<td valign="top"><label for="ar_description">Detail in Arabic</label></td>
