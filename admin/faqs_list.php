<?php 
	...
	if(isset($_GET['args1'])){
		...
		$mysql->record_update("faqs_tbl",array('status' => $args2),"id=$args1");
		//print_r($check);
	}	
	if(isset($_GET['args3'])){
		$args3 =cleaninputfield($mysql,$_GET['args3']);
		$mysql->record_delete("faqs_tbl","id = $args3");
	}
	?>
	...
	<div class="wrap">
		...
		<div class="container">
			...
			<table width="100%" border="0" cellspacing="0" cellpadding="0" class="tbl">
				  <tr class="headerbar">
					<th width="38%" class="alignleft">Question</th>
					<th width="13%" class="alignleft">Date</th>
					<th width="12%">Sort Order</th>
					<th width="12%">Status</th>
					<th width="11%">Edit FAQ </th>
					<th width="14%">Delete FAQ </th>
				  </tr>
				  <?php 
					$i=1;
					$tbl_name="faqs_tbl";		//your table name
					...
					if($lastpage > 1){	
						...
						foreach($postrows as $value => $postrow){
							$row_class= ($i%2==0) ? "class='alttr'" : '"alignleft"';
							?>
				  	<tr <?php echo $row_class; ?>>
					<td class="alignleft"><?php echo stripslashes($postrow['question']); ?></td>
					<td class="alignleft"><?php echo $postrow['faqdate']; ?></td>
					<td align="left"><?php echo stripslashes($postrow['sortorder']); ?></td>
					<td align="left">
					<?php 
						$status=$postrow['status'];
						if($status == 1){ 
							echo "<a href='faqs_list.php?args1=$postrow[id]&args2=0'> &nbsp; &nbsp; <img src='images/active.png' border='0' alt='active' /> Active &nbsp; &nbsp;</a>";
						}else{ 
							echo "<a href='faqs_list.php?args1=$postrow[id]&args2=1'> &nbsp; &nbsp; <img src='images/hidden.png' border='0' alt='hidden' /> Inactive</a>";
						}	
						?></td>
					<td><a href="new_faq.php?args1=<?php echo $postrow['id']; ?>"><img src="images/edituser.png" border="0" alt="edit user" title="Edit" /></a></td>
					<td><a href="faqs_list.php?args3=<?php echo $postrow['id']; ?>" onclick="return confirm('Do you want to delete this?');"><img src="images/delete.png" border="0" title="Delete" alt="delete user" /></a></td>
				  </tr>
				  <?php 
				  		$i++;
				  		} ?>
			...
			<div class="addbtn-right">
				<a href="new_faq.php" title="Add FAQ"><input name="add" id="add" border="0" type="submit" value="Add FAQ" class="addpagebtn" /></a> 
			</div>
		</div>
