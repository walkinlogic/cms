	...
	<div class="wrap">
		...
		<?php 
			if(isset($_POST['add'])){
				...
				$queryinsert = $mysql->record_insert("staff_tbl",array('ar_title' => $ar_title,'ar_description' => $ar_description,'description' => $description ,'title' => $title,'link' => $link ,'image' => $imagename,'sortorder' => $sortorder),false);
				if($queryinsert){
					$msg= base64_encode('Custom Image Added Successfully!');
					echo "<script> window.location='staff.php?msg=$msg';</script>";
					exit();
				}else{
			...
			if(isset($_POST['update'])){
				...
				if($_FILES['image']['tmp_name']!=''){
				 ...
				 $description =cleaninputfield($mysql,$_POST['description']);				
				 $ar_description =cleaninputfield($mysql,$_POST['ar_description']);
				$queryupdate = $mysql->record_update("staff_tbl",array('ar_description' => $ar_description,'description' => $description ,'title' => $title,'ar_title' => $ar_title,'link' => $link ,'image' => $imagename,'sortorder' => $sortorder),"id=$imageid");
				if($queryupdate){
			...
			if(isset($_GET['args1'])){
				...
				$postrow = $mysql->fetch_row("staff_tbl","id=$args1");
			}
			?>
				   ...
				   <tr>
				     ...
				     <td><?php 
						if(isset($postrow)){
							...
							$max_value = $mysql->table_max_value("staff_tbl","sortorder"); 
							$max_value += 1;
						}
						?>
